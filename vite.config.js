...
export default defineConfig(({mode})=>{
 ...
 }}}},plugins:[{name:'confirmed-site-seo',enforce:'post',configurePreviewServer(server){server.middlewares.use((request,_response,next)=>{const [path,query]=request.url.split('?');const route=path.replace(/\/$/,'');if(route&&Object.hasOwn(routeMetadata,route))request.url=route+'/index.html'+(query?'?'+query:'');next()})},configureServer(server){server.middlewares.use((request,response,next)=>{const path=request.url?.split('?')[0];if(path!=='/sitemap.xml'&&path!=='/robots.txt')return next();response.setHeader('Content-Type',path==='/sitemap.xml'?'application/xml; charset=utf-8':'text/plain; charset=utf-8');response.end(path==='/sitemap.xml'?sitemap:robots)})},transformIndexHtml(html,context){const organization={'@context':'https://schema.org','@type':'Organization',name:'Nandices Confeitaria',telephone:`+${PHONE}`,sameAs:['https://www.instagram.com/nandices.confeitaria/'],...(origin?{url:origin+'/'}:{})};html=html.replace('</head>',`<script type="application/ld+json">${JSON.stringify(organization)}</script></head>`);if(!origin)return html;const page=html.replace('</head>',`<link rel="canonical" href="${origin}/"/><meta property="og:url" content="${origin}/"/></head>`).replace('content="/images/products/degustacao/caixa-degustacao-960.webp"',`content="${origin}/images/products/degustacao/caixa-degustacao-960.webp"`);const path=(context.originalUrl||context.path||'/').split('?')[0].replace(/\/$/,'')||'/';return pageMetadataHtml(page,path,origin)},generateBundle(_options,bundle){if(origin){

...
export default defineConfig(({mode})=>{
 ...
 return {envPrefix:[],server:{proxy:{'/api':{target:'http://127.0.0.1:8787',changeOrigin:true,configure(proxy){
  ...
  proxy.on('error',(_error,_request,response)=>{if(!response.headersSent){response.writeHead(502,{'Content-Type':'application/json','Cache-Control':'no-store'});response.end(JSON.stringify({status:'unavailable',error:'Não conseguimos calcular o frete automaticamente. Consulte a entrega pelo WhatsApp.'}))}});

...
export default defineConfig(({mode})=>{
 ...
 }}}},plugins:[{name:'confirmed-site-seo',transformIndexHtml(html){const organization={'@context':'https://schema.org','@type':'Organization',name:'Nandices Confeitaria',telephone:`+${PHONE}`,sameAs:['https://www.instagram.com/nandices.confeitaria/'],...(origin?{url:origin+'/'}:{})};html=html.replace('</head>',`<script type="application/ld+json">${JSON.stringify(organization)}</script></head>`);if(!origin)return html;return html.replace('</head>',`<link rel="canonical" href="${origin}/"/><meta property="og:url" content="${origin}/"/></head>`).replace('content="/images/products/degustacao/caixa-degustacao-960.webp"',`content="${origin}/images/products/degustacao/caixa-degustacao-960.webp"`)},generateBundle(){if(origin)this.emitFile({type:'asset',fileName:'sitemap.xml',source:`<?xml version="1.0" encoding="UTF-8"?>

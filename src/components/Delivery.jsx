...
const ADDRESS_REVIEW='Não conseguimos confirmar esse endereço com segurança. Revise os dados informados ou consulte a entrega pelo WhatsApp.';
export default function Delivery({orderContext=''}){
 ...
 async function calculate(e){
  ...
  if(address.address.trim().length<5||!address.number.trim()){setError(ADDRESS_REVIEW);return}
  ...
  try{
   ...
   if(!response.ok||data.status!=='estimated')throw new Error(data.status==='address_review_required'?ADDRESS_REVIEW:FALLBACK);
   if(!Number.isSafeInteger(data.estimatedCents)||data.estimatedCents<0||!Number.isFinite(data.distanceKm)||data.distanceKm<0||!Number.isFinite(data.billableDistanceKm)||data.billableDistanceKm<data.distanceKm||typeof data.destination!=='string'||!data.destination.trim())throw new Error(FALLBACK);

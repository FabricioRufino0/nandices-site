...
export default function Configurator({lots,setLots,open,onToggle,onContinue}){
 ...
 return <ExpandableSection id="configurador" titleId="config-title" title="Monte sua encomenda." label="DO SEU JEITO" className="config" open={open} onToggle={onToggle}>
  ...
  {summary.complete?<button type="button" className="button" onClick={onContinue}>Consultar minha encomenda <span aria-hidden="true">↓</span></button>:<button className="button" disabled>Selecione o sabor de todos os lotes</button>}<small>Valor calculado dos doces, sem frete. Sujeito à confirmação pela Nandices.</small></div>
 </ExpandableSection>;
}

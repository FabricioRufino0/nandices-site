...
const entries = [
 ...
 ['casadinho','Casadinho','Tradicional',false,'Brigadeiro tradicional + brigadeiro de Ninho. Finalização no leite em pó.'],
 ...
 ['pistache','Pistache','Pistache',false,'Recheio realizado com pistache em grãos, sem pastas ou cremes de pistache prontos. Finalização com pistache xerém.']
];
const studioPhotos = new Set(['ferrero-rocher','maracuja','creme-brulee','cajuzinho','pistache']);
export const sweets = entries.map(([id,name,category,featured,description]) => ({id,name,category,featured,description,composition:description,ingredients:[],highlightIngredients:[],priceGroup:category,available:null,image:`/images/products/brigadeiros/${id}${['pistache','maracuja'].includes(id)?'-reference':studioPhotos.has(id)?'-studio':''}`}));
export const cakes = [
 ...
 {id:'baba-de-moca',name:'Baba de Moça com Frutas Vermelhas',description:'Massa de pão de ló, molhada no leite, recheio de baba de moça e geleia caseira de frutas vermelhas. Finalização com frutas vermelhas.',image:'/images/products/bolos/bolo-frutas-vermelhas'},

export const FALLBACK = 'Não conseguimos calcular o frete automaticamente. Consulte a entrega pelo WhatsApp.';
export const ADDRESS_REVIEW = 'Não conseguimos confirmar esse endereço com segurança. Revise os dados informados ou consulte a entrega pelo WhatsApp.';
...
class DeliveryError extends Error {
  constructor(status = 422, review = true) { super('Delivery unavailable'); this.status = status; this.review = review; }
}
...
async function orsJson(fetcher, url, options, signal) {
  ...
  if (!response.ok) throw new DeliveryError(502, false);
  return response.json();
}
async function geocode(text, key, fetcher, signal, number) {
  ...
  if (!Array.isArray(features) || !features.length) throw new DeliveryError();
  ...
  if (p.country_a !== 'BRA' || !(['DF', 'BR-DF', 'Distrito Federal'].includes(p.region_a) || p.region === 'Distrito Federal') || !['address', 'venue'].includes(p.layer) || p.accuracy !== 'point' || p.match_type !== 'exact') throw new DeliveryError();
  ...
  // A precise point for another house must not be used for the requested number.
  if (number && (typeof p.housenumber !== 'string' || p.housenumber.trim().toLowerCase() !== number.toLowerCase())) throw new DeliveryError();
...
export async function handleDelivery(request, env, fetcher = fetch) {
  ...
  try {
    ...
    if (address.length < 5 || address.length > 300 || !number || number.length > 30 || complement.length > 150) return failure(400, true);
    const config = configuration(env), signal = AbortSignal.timeout(18000);
    const destination = await geocode(`${address}, ${number}, Distrito Federal, Brasil`, env.OPENROUTESERVICE_API_KEY, fetcher, signal, number);
    ...
    return reply({status: 'estimated', estimatedCents, distanceKm, billableDistanceKm, tripMode: config.tripMode, destination: destination.label, notice: 'Valor estimado, sujeito à confirmação pela Nandices.'});
  } catch (error) {

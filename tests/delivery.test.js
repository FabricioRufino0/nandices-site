...
const feature=(id,label='Rua de teste, DF',confidence=.9,state='Distrito Federal')=>({type:'Feature',geometry:{type:'Point',coordinates:id==='origin'?[-47.9,-15.8]:[-47.8,-15.7]},properties:{id,label,confidence,country_a:'BRA',region_a:state,layer:'address',accuracy:'point',match_type:'exact',housenumber:'12'}});
...
for(const side of ['destination','origin'])for(const [name,mutate] of Object.entries(uncertainCases))test(`${side}: rejeita ${name} antes de consultar rotas`,async()=>{
...
test('número divergente ou ausente não produz estimativa',async()=>{for(const value of ['13',undefined]){
 const destination=feature('destination');destination.properties.housenumber=value;
 assert.equal((await handleDelivery(request(),env,fakeORS({features:[destination,feature('origin')]}).fetcher)).status,422);
}});
test('falhas têm envelope consistente e nunca dados parciais ou privados',async()=>{
 ...
 for(const pending of cases){const res=await pending;const body=await res.json();assert.deepEqual(Object.keys(body).sort(),['error','status']);assert.match(body.error,/WhatsApp/);for(const value of PRIVATE_BINDINGS.map(k=>env[k]))assert.ok(!JSON.stringify(body).includes(value));}
});

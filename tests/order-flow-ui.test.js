...
for(const width of [390,1440])test(`fluxo de encomenda e blocos expansíveis em ${width}px`,async()=>{
 ...
 try{
  ...
  await expect(page.locator('.instagram-cta')).toHaveAttribute('href','https://www.instagram.com/nandices.confeitaria/');
  await expect(page.locator('#bolos .custom-highlight a')).toHaveAttribute('href','#personalizados');
  ...
  await expect(page.locator('#como-encomendar li')).toHaveCount(3);
  const help=page.locator('.catalog-help').getByRole('link',{name:'Falar com a Nanda'});
  const helpUrl=new URL(await help.getAttribute('href'));
  assert.equal(helpUrl.host,'wa.me');assert.equal(helpUrl.pathname,'/5561993359461');
  assert.equal(helpUrl.searchParams.get('text'),'Olá! Vim pelo site da Nandices e estou em dúvida sobre qual opção escolher. Pode me ajudar?');
  ...
  assert.deepEqual(await page.locator('#sobre,#como-encomendar,#bolos,#doces,#quanto-pedir,#configurador,#degustacao,#personalizados,#entrega').evaluateAll(elements=>elements.map(e=>e.id)),['sobre','como-encomendar','bolos','doces','quanto-pedir','configurador','degustacao','personalizados','entrega']);
  ...
  assert.equal(photos.length,12);assert.ok(photos.every(p=>p.fit==='contain'&&p.position==='50% 50%'&&Math.abs(p.ratio-1)<.01));
  await configToggle.click();await plannerToggle.click();await planner.scrollIntoViewIfNeeded();await page.screenshot({path:`docs/order-flow-${width}.png`});

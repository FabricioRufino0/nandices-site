...
test('mensagem WhatsApp mantém nomes, quantidades, forminhas, total e confirmação',()=>{
 const message=configurationMessage([lot('ninho-com-nutella','Pistache'),lot('ninho-com-nutella','Chocolate'),lot('brigadeiro-tradicional')]);
 for(const expected of ['Quantidade: 150 doces','100 Ninho com Nutella','50 com forminha Pistache','50 com forminha Chocolate','50 Brigadeiro Tradicional','315,00','confirmar disponibilidade'])assert.ok(message.includes(expected),expected);
...
for(const [id,min,max] of references)test(`planejador ${id}: referências e arredondamento`,()=>{
 for(const guests of [1,10,20,40,50,67,200,1000]){
  ...
  assert.equal(result.cakeKg,Math.max(1.5,guests/10));assert.equal(result.rawMin,guests*min);assert.equal(result.rawMax,guests*max);
...
test('mensagens do planejador e entrega mantêm contexto',()=>{assert.match(plannerMessage(planEvent('aniversario',40)),/150 a 200/);const text=deliveryMessage('delivery',{address:'Rua de teste',number:'12',complement:'Bloco A'},{distanceKm:12,estimatedCents:1200});assert.match(text,/Bloco A/);assert.match(text,/12,00/);assert.match(text,/confirmar/);assert.doesNotMatch(deliveryMessage('pickup',{},null),/undefined/)});
